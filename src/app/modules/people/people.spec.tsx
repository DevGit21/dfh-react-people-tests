...
const mockPeopleData = [
  { id: 1, name: "Addie Duncan", show: "Show 1", actor: "Actor 1", dob: "1990-01-01", movies: [{ title: "Movie A" }] },
  { id: 2, name: "Zelma Mcdaniel", show: "Show 2", actor: "Actor 2", dob: "1991-02-02", movies: [{ title: "Movie B" }] },
  { id: 3, name: "Ball Higgins", show: "Show 3", actor: "Actor 3", dob: "1992-03-03", movies: [{ title: "Movie C" }] },
  { id: 4, name: "Singleton Ball", show: "Show 4", actor: "Actor 4", dob: "1993-04-04", movies: [{ title: "Movie D" }] },
  { id: 5, name: "Sammy Davis", show: "Show 5", actor: "Actor 5", dob: "1994-05-05", movies: [{ title: "Movie E" }] },
  { id: 6, name: "Danny Sullivan", show: "Show 6", actor: "Actor 6", dob: "1995-06-06", movies: [{ title: "Movie F" }] },
  { id: 7, name: "Amy Moore", show: "Show 7", actor: "Actor 7", dob: "1996-07-07", movies: [{ title: "Movie G" }] },
  { id: 8, name: "Ella Stone", show: "Show 8", actor: "Actor 8", dob: "1997-08-08", movies: [{ title: "Movie H" }] },
  { id: 9, name: "Mark Ball", show: "Show 9", actor: "Actor 9", dob: "1998-09-09", movies: [{ title: "Movie I" }] },
  { id: 10, name: "Nancy Brown", show: "Show 10", actor: "Actor 10", dob: "1999-10-10", movies: [{ title: "Movie J" }] }
];
...
describe("People Component", () => {
  ...
  test("handles error response", async () => {
    ...
    await renderPeople();
    expect(screen.getByRole("heading", { name: "Oops! looks like something went wrong!" })).toBeInTheDocument();
  });
  ...
  describe("Filtering", () => {
    test("should select the sort column", async () => {
      ...
      expect(screen.getByRole("table")).toBeInTheDocument();
      expect(within(screen.getAllByRole("row")[1]).getByText("Addie Duncan")).toBeInTheDocument();
      expect(screen.getByRole("columnheader", { name: "Name" })).toHaveAttribute("aria-sort", "ascending");

      await user.click(screen.getByRole("columnheader", { name: "Name" }));
      expect(screen.getByRole("columnheader", { name: "Name" })).toHaveAttribute("aria-sort", "descending");
      expect(within(screen.getAllByRole("row")[1]).getByText("Zelma Mcdaniel")).toBeInTheDocument();
    });
    ...
    test("should filter the list by name", async () => {
      ...
      expect(within(screen.getAllByRole("row")[1]).getByText("Ball Higgins")).toBeInTheDocument();
      expect(within(screen.getAllByRole("row")[2]).getByText("Singleton Ball")).toBeInTheDocument();
    });
  });
  ...
  describe("Pagination", () => {
    test("should update number of people displayed", async () => {
      ...
      const mockPeopleData = new Array(100).fill(null).map((_, index) => ({
        name: `Person ${index + 1}`, show: `Show ${index + 1}`, actor: `Actor ${index + 1}`, dob: "01/01/1990", movies: [{ title: `Movie ${index + 1}` }]
      }));
      server.use(http.get(getPeople.info.path, () => HttpResponse.json(mockPeopleData)));
