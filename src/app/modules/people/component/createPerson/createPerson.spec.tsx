...
describe("CreatePerson Component", () => {
  test("renders form", async () => {
    ...
    expect(screen.getByRole("heading", { name: /create new person/i })).toBeInTheDocument();
  ...
  test("fills out form and submits", async () => {
    ...
    fireEvent.change(screen.getByLabelText(/show/i), { target: { value: 'Show 1' } });
    ...
    fireEvent.change(screen.getAllByPlaceholderText(/movie title/i)[0], { target: { value: 'Movie 1' } });
    fireEvent.change(screen.getAllByPlaceholderText(/release date/i)[0], { target: { value: '2020-12-01' } });

    // Submit the form
    fireEvent.click(screen.getByText(/Create Person/i));

    // Success message
    expect(screen.getByText(/Person created successfully!/i)).toBeInTheDocument(); 
  });
});
